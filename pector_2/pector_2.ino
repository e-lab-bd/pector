...
static bool readModbusValues(uint16_t& moist, uint16_t& ec, uint16_t& ph, uint16_t& temp, uint8_t& errOut) {
  ...
  return true;
#else
  uint8_t res = node.readHoldingRegisters(0, 3);
  ...
  ph = 0;  // This sensor doesn't have any pH sensing probe, so 0 will be shown as default
