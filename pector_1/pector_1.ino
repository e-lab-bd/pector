...
/* MODE */
enum class RunMode : uint8_t { Normal,
                               Ota };
...
static void sendEcOverTcp(uint16_t moist_tcp, uint16_t temp_tcp, uint16_t ec_tcp, uint16_t ph_tcp, uint16_t nitro_tcp, uint16_t phosp_tcp, uint16_t potas_tcp) {
  if (tcpClient && tcpClient.connected()) {
    tcpClient.printf("MO=%u TMP=%u EC=%u pH=%u N=%u P=%u K=%u\n", moist_tcp, temp_tcp, ec_tcp, ph_tcp, nitro_tcp, phosp_tcp, potas_tcp);
    lastTcpActivity = millis();
  }
}

/* MODBUS READ */
static bool readModbusValues(uint16_t& moist, uint16_t& temp, uint16_t& ec, uint16_t& ph, uint16_t& nitro, uint16_t& phosp, uint16_t& potas, uint8_t& errOut) {
#if EMULATE_MODBUS
  (void)errOut;

  static uint16_t em = 400, et = 250, ee = 1000, ep = 65, en = 10, epp = 20, ek = 30;
  em = (em + 5) % 1000;
  et = 200 + (millis() / 1000) % 80;
  ee = (ee + 13) % 3000;
  ep = 60 + (millis() / 1000) % 20;
  en = 10 + (millis() / 1000) % 50;
  epp = 20 + (millis() / 1000) % 50;
  ek = 30 + (millis() / 1000) % 50;
  ...
  return true;
#else
  uint8_t res = node.readHoldingRegisters(0, 7);
  if (res != node.ku8MBSuccess) {
    errOut = res;
    return false;
  }

  uint16_t moist_local = node.getResponseBuffer(0);
  uint16_t temp_local = node.getResponseBuffer(1);
  uint16_t ec_local = node.getResponseBuffer(2);
  uint16_t ph_local = node.getResponseBuffer(3);

  uint16_t nitro_local = node.getResponseBuffer(4);
  uint16_t phosp_local = node.getResponseBuffer(5);
  uint16_t potas_local = node.getResponseBuffer(6);
...
void loop() {
  ...
  if (millis() - lastPoll >= 2000) {
    ...
    uint16_t moist = 0, temp = 0, ec = 0, ph = 0, nitro = 0, phosp = 0, potas = 0;
    uint8_t mbErr = 0;

    if (readModbusValues(moist, temp, ec, ph, nitro, phosp, potas, mbErr)) {
      Serial.printf("MO=%u TMP=%u EC=%u pH=%u N=%u P=%u K=%u\n", moist, temp, ec, ph, nitro, phosp, potas);
      onModbusOk(moist, ec, ph);
      sendEcOverTcp(moist, temp, ec, ph, nitro, phosp, potas);
    } else {
      onModbusError(mbErr);
    }
  }
